...
import { get_default_key } from "./dbroot.ts";

/**
 * 使用密钥解码数据
 *
 * @param {Uint8Array} encrypted_data - 待解码的原始数据（Uint8Array类型）
 * @param {Uint8Array} key - 密钥（从DBroot文件中获取，长度为1024字节）
 * @returns {Uint8Array} 解码后的数据，若输入为空则返回空数组
 *
 * @example
 * 基本用法
...
export function decode_data(
  ...
  if (!key) {
    key = get_default_key(); // 使用默认密钥
  }
  // 创建一个新的 Uint8Array 来存储解码后的数据
...
export function decode_tile(
  ...
  if (!array_is_equal(tile_data.slice(0, 4), header)) {
    throw new Error("can not decode tile data, invalid header");
  }
